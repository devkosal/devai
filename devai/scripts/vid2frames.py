...
def convert_one(video_file):
    ...
    while success:
        ...
        cv2.imwrite(str(output_path/frame_num)+".jpg", image)     # save frame as JPEG file

        success,image=vidcap.read() # changing success, image for the next iteration
        count += 1

def videos_to_frames_converter(input_path, include=["avi","mp4","mov","wmv","mkv","flv"]):
    ...
    if input_path.is_file():
        ...
        for video_file in tqdm(input_path.ls(include=["avi"])):
